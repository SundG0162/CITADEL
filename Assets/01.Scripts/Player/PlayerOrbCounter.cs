...
[Serializable]
public class PlayerOrbCounter : MonoBehaviour
{
    ...
    [SerializeField]
    private List<OrbSet> _orbList = new List<OrbSet>();
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.R) && UIManager.Instance.fireOrb > 0)
        {
            transform.parent.GetComponent<BoxCollider2D>().enabled = false;
            RotationSpeedDown();
            UIManager.Instance.FireOrbDown();
            lightning = false;
            fire = true;
            AddOrb();

        }
        else if (Input.GetKeyDown(KeyCode.T) && UIManager.Instance.lightningOrb > 0)
        {
            transform.parent.GetComponent<BoxCollider2D>().enabled = false;
            RotationSpeedDown();
            UIManager.Instance.LightningOrbDown();
            fire = false;
            lightning = true;
            AddOrb();
        }
        else if (Input.GetKeyDown(KeyCode.Q) && !TowerManager.Instance.blueTowerActive)
        {
            RotationSpeedUp();
        }
        else if (Input.GetKeyUp(KeyCode.Q) && !TowerManager.Instance.blueTowerActive)
        {
            RotationSpeedDown();
        }
        ...
        if(UIManager.Instance._gaugeValue <= 0)
        {
            RotationSpeedDown();
        }
    }
    ...
    private void RotationSpeedUp()
    {
        ...
        UIManager.Instance.GaugeControll(-2);
    }
    ...
    public void RotationSpeedDown()
    {
        ...
        UIManager.Instance.GaugeControll(1f);
    }
    ...
    private void OrbSpawn()
    {
        if (fire)
        {
            ...
            _orbList.Add(new OrbSet { OrbIndex = numberOfObjects, InitPos = Vector3.zero, OrbInstance = obj });
            numberOfObjects++;
        }
        else if (lightning)
        {
